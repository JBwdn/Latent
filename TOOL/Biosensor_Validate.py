...
input_file = 'biosensor_seqs_annotated.csv'
sidx = {}
...
with open('param.pkl', 'rb') as fp:
    d_to_index = pickle.load(fp)
...
for i in sorted( np.arange(0, len(ytest)) , key = lambda x: -ytest[x] ):
    s = cvtest.iloc[i]['seq']
    print(  ytest[i][0], target, sidx[s][3], sidx[s][5], sidx[s][6], sidx[s][7] )
    h += 1
    if h>10:
        break
